...
public class MessageProcessingError {
    ...
    /**
     * Returns a concise, single-line summary, truncating details to the given length.
     */
    public String summary(int maxDetailLength) {
        StringJoiner sj = new StringJoiner(" | ");
        if (!isBlank(eventType)) sj.add("event=" + eventType);
        if (!isBlank(entityType)) sj.add("entity=" + entityType);
