...
/**
 * Entidad JPA para el log de idempotencia de copia de kardex weighted-average.
 * DDL PostgreSQL: TIMESTAMPTZ + UNIQUE (id_proceso, fase, modulo).
 * REQ-INVWA-01, ADR-38.
 */
@Entity
@Table(name = "kardex_wa_copy_job_log",
    uniqueConstraints = {
        @UniqueConstraint(name = "uq_kardex_wa_copy_job_log",
                columnNames = {"id_proceso", "fase", "modulo"})
    },
    indexes = {
        @Index(name = "idx_kardex_wa_copy_job_log_id_proceso", columnList = "id_proceso")
    }
)
@Data
@Builder
